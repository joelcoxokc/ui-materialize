;(function() { 'use strict';
    ...
    function $mzConfig() {
        var _this = this;
        _this.navs = { top:{} , bottom:{} , right:{} , left:{} };
        _this.navProps =
            { top   : { small  : {name:'small' , scope:'isSmall' , val:true , class:'nav-top-small'    , event:'$navTopIsSmall'    , unToggleOnToggle: ['isMedium', 'isLarge' ], onToggle:'onToggleSmall'   , onChange:function(value,cb){}           }
            ...
            , right : { open   : {name:'open'  , scope:'isOpened', val:true , class:'nav-right-open'   , event:'$navRightOpened'  /*   *   *   *   *   *   *   *   *   *   */ , onToggle:'onRightNavOpened' , onChange:function(value){}              }
            ...
            , bottom: { small  : {name:'small' , scope:'isSmall' , val:true , class:'nav-bottom-small' , event:'$navBottomIsSmall' , unToggleOnToggle: ['isMedium', 'isLarge' ], onToggle:'onToggleSmall'   , onChange:function(value){}              }
            ...
            , left  : { open   : {name:'open'  , scope:'isOpened', val:false, class:'nav-left-open'    , event:'$navLeftOpened'    , unToggleOnToggle: ['isFolded'            ], onToggle:'onLeftNavOpened' , onChange:function(value){}              }
        ...
        _this.createToggle = function (nav, property) {
            ...
            return function ( callback ) {
                nav[property.onToggle] = function (callback){
                    _.forEach(this.unToggleOnToggle, function (item){ nav[item] = false; });
                    ...
                    nav._$onToggle &&( nav._$onToggle(nav[toggleProperty.scope], oldValue) );
                    callback       &&( callback(      nav[toggleProperty.scope], oldValue) );
                    return nav.val;
                  };
              };
          }; // end function _this.createToggle

        _this.conditions =
            { top   : { add:'$hasNavTop'  }
            , right : { add:'$hasAsideRight' , open:'$isRightOpen' , close:'$isRightClosed' , fold:'$isLeftFolded' }
            , bottom: {}
            , left  : { add:'$hasAsideLeft'  , open:'$isLeftOpen'  , close:'$isLeftClosed'  , fold:'$isLeftFolded' }
            };
        _this.events =
            { top   : { add:'$navAdded' }
            , right : { add:'$navAdded'      , open:'$navOpened'   , close:'$navClosed'     , fold:'$navFolded'    }
            , bottom: { add:'$navAdded' }
            , left  : { add:'$navAdded'      , open:'$navOpened'   , close:'$navClosed'     , fold:'$navFolded'    }
            };
        _this.classNames =
            { top   : { open:'nav-top-open'   , close:'nav-top-close'   ,    sm:'nav-top-sm'     , md:'nav-top-md'     , lg:'nav-top-lg'     }
            , right : { open:'nav-right-open' , close:'nav-right-close'   ,  fold:'nav-right-fold' }
            , bottom: { open:'nav-bottom-open', close:'nav-bottom-close',    sm: 'nav-bottom-sm' , md: 'nav-bottom-md' , lg: 'nav-bottom-lg' }
            , left  : { open:'nav-left-open'  , close:'nav-left-close'    ,  fold:'nav-left-fold'  }
            };
        ...
        _this.createNav = function(nav) {
            ...
            nav.$toggle = function() { nav.$toggles(cb); };
          };
        ...
        _this.configNavs = function(nav) {
            _.forEach(_this.navs, function (nav, type) {
                ...
                nav.$toggle = function(name, cb) { nav.$toggles(cb); };   });
