;(function() { 'use strict';
    ...
    function RightNavigationService($NavService) {
        ...
        function RightNavigation(side, element, attrs, config) {  $NavService.apply(this, arguments);  var self = this;  };

        function protoActivate() {
            var _this = this;
            this.startWatch();
            _.forEach(this.classes, function (className) { _this.element.addClass(className) });
          };

        function protoOpen() {
            this.element.addClass(this.classes.open);
            this.config.fold &&( this.unfold() );
            this.broadcast('nav:'+this.side+':opened');
          };

        function protoClose() {
            this.element.removeClass(this.classes.open);
            this.config.fold &&( this.fold() );
        ...
        function protoStartWatch(value, classList) {
            this.watch( 'isOpen' , function (value){value?this.open():this.close();} );   };
        function protoUseLargeAction() {  this.element.addClass('has-large-action');  };
        
        /// service helpers

        function createClass(side) { return 'nav-' + side; }

      } // end function RightNavigationService
